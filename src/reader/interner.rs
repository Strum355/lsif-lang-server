...
impl Interner {
    ...
    pub fn intern(&self, raw: &[u16]) -> Result<u64, std::num::ParseIntError> {
        if raw.is_empty() {
            return Ok(0)
        }

        let mut array = [0; 1];
        let double_quotes = '"'.encode_utf16(&mut array);
 
        if raw[0] != double_quotes[0] {
            return String::from_utf16(raw).unwrap().parse::<u64>()
        }

        let s = String::from_utf16(&raw[1..raw.len()-1]).unwrap();
...
mod tests {
    ...
    fn compare_from_vec(input: &[Vec<u16>]) -> Result<HashSet<u64>> {
        ...
        for num in input {
            let x = interner.intern(num).unwrap();
            results.get(&x).ok_or(format_err!("result not found in previous set"))?;
        }

        Ok(results)
    }

    fn string_vec_to_utf16(values: Vec<&str>) -> Vec<Vec<u16>> {
    ...
    #[test]
    fn numbers_test() {
        let values = string_vec_to_utf16(vec![
            "1", "2", "3", "4", "100", "500"
        ]);
